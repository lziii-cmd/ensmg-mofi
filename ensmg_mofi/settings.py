...
SECRET_KEY = os.environ.get(
    "SECRET_KEY", "django-insecure-ensmg-mofi-dev-key-change-in-production-2024"
)
...
EMAIL_BACKEND = os.environ.get(
    "EMAIL_BACKEND", "django.core.mail.backends.console.EmailBackend"  # console par défaut (dev)
)
...
# ── Cache ─────────────────────────────────────────────────────────────────────
# En production, définir REDIS_URL pour utiliser Redis (requis pour le rate-limiting multi-workers)
...
# Sinon, LocMemCache (dev single-process) — l'alerte django_ratelimit.E003 est silencée
SILENCED_SYSTEM_CHECKS = ["django_ratelimit.E003", "django_ratelimit.W001"]

# ── Rate limiting ─────────────────────────────────────────────────────────────
# Désactivé automatiquement en test (RATELIMIT_ENABLE = False dans pytest.ini)
